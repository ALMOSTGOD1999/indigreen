...
export default function AdminDashboardPage({ stats, goldPrice }: AdminDashboardProps) {
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: Calendar03Icon,
      gradient: 'from-copper/20 via-copper/5 to-transparent',
      border: 'border-copper/20',
      iconBg: 'bg-copper/10 text-copper',
  ...
  return (
    <>
      ...
      <AppLayout>
        ...
        <Main>
          {goldPrice && (
            <Alert className="mb-6 border-emerald/20 bg-gradient-to-r from-emerald/5 to-transparent overflow-hidden rounded-xl">
              <AlertTitle className="w-full overflow-hidden text-emerald font-semibold flex items-center gap-2">
                <span className="inline-flex size-6 items-center justify-center rounded-full bg-emerald/20 text-emerald text-xs">
                  ₹
                </span>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {statCards.map((stat, i) => (
              <div
                key={i}
                className={`relative overflow-hidden rounded-2xl border ${stat.border} bg-gradient-to-br ${stat.gradient} p-6 backdrop-blur-sm transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5`}
              >
