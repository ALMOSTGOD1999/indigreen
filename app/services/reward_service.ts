...
export default class RewardService {
  ...
  // Activation Cashback: 10% of activation amount (999), split 5%+5% over 2 months
  static async getActivationCashbackRewards(
    ...
    const ACTIVATION_AMOUNT = Number(user.activationAmount) || 999
  ...
  static async getUserMonthlyWorkingIncome(user: User, month: DateTime): Promise<number> {
    ...
    if (user.activatedAt) {
      const actAmt = Number(user.activationAmount) || 999
    ...
    if (hasDirects) {
      ...
      const sponsorCountRes = await user
        ...
        .count('* as total')
      total +=
        Number(sponsorCountRes[0].$extras.total) * ((Number(user.activationAmount) || 999) * 0.1)
