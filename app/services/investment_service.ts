...
export default class InvestmentService {
  ...
  /**
   * Check if investment has reached its maximum return cap.
   * Primary check: max months elapsed since startedAt.
   * Fallback: total return >= maxReturnPercent of investment amount.
   */
  static async hasReachedMaxReturn(investment: Investment): Promise<boolean> {
    ...
    // Check max months first
    if (pkg.maxMonths) {
      const startedAt = DateTime.fromJSDate(new Date(investment.startedAt.toString()))
      const monthsElapsed = DateTime.now().diff(startedAt, 'months').months
      if (monthsElapsed >= pkg.maxMonths) return true
    }

    // Fallback: check total return against maxReturnPercent
  ...
  static async distributeMonthlyReturns(
    ...
    for (const investment of investments) {
      ...
      if (reachedMax) {
        ...
        investment.remark = 'Maximum return period reached'
